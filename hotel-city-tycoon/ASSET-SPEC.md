...
- `_thumb` — 96×96: الغرفة كاملة مركّبة على بطاقة موحّدة، لا قصّ لربعها. تشمل
  طبقة `_front` إن وُجدت، لأنها صورة **عن** الغرفة لا طبقة **منها**.
- `_front` — **اختيارية**، وليست مشتقة: الأثاث الذي يقف الناس **خلفه** (مكتب
  الاستقبال، كاونتر البار)، على canvas شفافة يرسمها `RoomSpec.front`. يرسمها
  العارض في طبقة فوق الشخصيات (`LAYER.roomFront`)، فما دخل هنا يحجب من خلفه —
  ولذلك لا شيء فيها إلا الأثاث نفسه، ولا جدار ولا أرض. تمتد تحت خط الأرض بمقدار
  `FOOT_OVERHANG` لتغطي حذاء وظلّ من يقف خلفها. غرفة بلا واجهة لا تشحن الملف
  ولا يعلنه الـmanifest (BL-035، `docs/BL-035-REPORT.md`).
