...
**سجل مؤرخ 21-09-2026 (DEC-024، HC-P2-S4A — «كبر الرقم»):** ثُبِّت حجم الرقم العائم `+N` بالشاشة بدل العالم. وهذا المسار **ليس الحالة المحايدة بل القصوى**: `bootEffects` يقلع بـ`?stress=12` الذي يوسّع القطعة إلى 15×10، وعالمها 2176×1248 بكسل، فـ`fitZoom` أصغر من أرضية الكاميرا على المشروعين ويُثبَّت عند `MIN_ZOOM = 0.4` — أي عند **أقصى مقياس** `labelScaleFor(0.4) = LABEL_SCALE_MAX = 4`. فما يقيسه اختبار «the effects put pixels on the canvas» هو الصورة **الجديدة** لا القديمة، وأُضيف إليه تأكيدان يقرآن `hct.fxStats()`: `labelScale > 1` (الكاميرا وصلت إلى الأرقام) و`labelScaleDrawn === labelScale` (المقياس المخزَّن وصل فعلًا إلى Sprite حيّ، لا إلى متغيّر وحده). عتبة الذهب `≥ 6` لكل 10000 **لم تُلمَس**: هي أرضية، والكمية التي تقيسها ارتفعت. وأُضيف اختبار ثالث، **«a number already in the air is re-laid out when the camera zooms»**، وهو الوحيد الذي يصل إلى حلقة إعادة الرصف في `setZoom`: يُطلق دفعة ثم يرسل اثنتي عشرة درجة قرص عجلة **والرقم ما زال في الهواء** ثم يقرأ الرقمين، فيمسك رقمًا احتفظ بحجمه القديم خلال قرصة. جُرّب على كود مكسور فاحمرّ بالرسالة الصحيحة. **تشغيل S4A على هذا المسار** (`PLAYWRIGHT_CHROMIUM_PATH=/opt/pw-browsers/chromium PLAYWRIGHT_EXTRA_ARGS=--disable-3d-apis PLAYWRIGHT_NO_CAPTURE=1`، خادم `webServer` الخاص على 5000، `s4a-e2e5.log`): **84 ناجحًا / 0 فاشل / 12 متخطاة في 3.7 دقيقة** — خط أساس S4 كان 82 والزائدان هما الاختبار الجديد على المشروعين، والمتخطاة هي الاثنتا عشرة نفسها بالاسم. **وحدّ صدق:** شُغّلت الدورة أولًا **بلا** `PLAYWRIGHT_EXTRA_ARGS` فسقط أحد عشر اختبارًا وقماشتها بيضاء والشارة تقول `renderer webgl` — وهو عين ما يصفه التصحيح المؤرخ 20-09-2026 أعلاه عن مسار SwiftShader الافتراضي؛ خطأُ تشغيل لا انحدار، ويُسجَّل هنا لأن أحدًا سيكرّره.
